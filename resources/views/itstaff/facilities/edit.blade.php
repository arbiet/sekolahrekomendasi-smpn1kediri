                ...
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('facilities.update', $facility->id) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="school_target_id" class="mt-1 block w-full border p-2 rounded">
                                @foreach($schools as $school)
                                    <option value="{{ $school->id }}" {{ $facility->school_target_id == $school->id ? 'selected' : '' }}>{{ $school->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="facility_name" value="{{ $facility->facility_name }}" class="mt-1 block w-full border p-2 rounded">
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="facility_description" class="mt-1 block w-full border p-2 rounded">{{ $facility->facility_description }}</textarea>
                        </div>
